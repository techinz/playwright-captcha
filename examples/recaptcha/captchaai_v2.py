...
CAPTCHA_AI_API_KEY = os.getenv('CAPTCHA_AI_API_KEY')  # replace with your CaptchaAI API key


async def solve_recaptcha_v2() -> None:
    ...
    async with async_playwright() as playwright:
        ...
        async with CaptchaAISolver(
                framework=framework,
                page=page,
                async_captcha_ai_client=AsyncCaptchaAI(CAPTCHA_AI_API_KEY)) as solver:
            await page.goto('https://2captcha.com/demo/recaptcha-v2')

            # wait for the page to load completely (could be replaced with a more robust check in a real scenario)
            await asyncio.sleep(5)

            # search for the element that contains the recaptcha challenge
            recaptcha_v2_container = page.locator('#g-recaptcha')
            await recaptcha_v2_container.wait_for()
            if not recaptcha_v2_container:
                logging.error('Recaptcha V2 container not found on the page')
                return
