...
@pytest.mark.asyncio
class TestRecaptchaTenCaptchaSolvers:
    """Integration tests for reCAPTCHA TenCaptcha solvers with all frameworks"""

    async def test_tencaptcha_recaptcha_v2_solver(self, browser_context, ten_captcha_client):
        """Test TenCaptcha solver for reCAPTCHA v2 with all frameworks"""
        ...
        async with TenCaptchaSolver(
                framework=framework,
                page=page,
                async_ten_captcha_client=ten_captcha_client
        ) as solver:
            ...
            # tencaptcha should return a token
            assert result is not None, "TenCaptcha solver should return a token"
        ...
        try:
            ...
            pytest.fail(f"TenCaptcha solver failed to solve reCAPTCHA v2 with {framework.value}: {e}")

    async def test_tencaptcha_recaptcha_v3_solver(self, browser_context, ten_captcha_client):
        """Test TenCaptcha solver for reCAPTCHA v3 with all frameworks"""
        ...
        try:
            ...
            pytest.fail(f"TenCaptcha solver failed to solve reCAPTCHA v3 with {framework.value}: {e}")

    async def test_recaptcha_solver_error_handling(self, browser_context, ten_captcha_client):
